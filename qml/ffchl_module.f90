...
function get_twobody_weights_ef(x, field, neighbors, power, cut_start, cut_distance, dim1, df) result(ksi)
    ...
    do i = 2, neighbors
        ...
        ksi(i) = cut_function(x(1, i), cut_start, cut_distance) & 
            & * (1.0d0/ x(1, i)**power + dot_product(dipole, field))
        ! ksi(i) = cut_function(x(1, i), cut_start, cut_distance) & 
...
function get_ksi_ef(x, na, nneigh, two_body_power, cut_start, cut_distance, df) result(ksi)
            ...
            do pm = 1, 2
                ...
                do i = 1, ni
                    ...
                    ksi(a, xyz, pm, i, :) = get_twobody_weights_ef(x(a,i,:,:), field, nneigh(a, i), &
                        & two_body_power, cut_start, cut_distance, maxneigh, df)
...
subroutine init_cosp_sinp_ef(x, na, nneigh, three_body_power, order, cut_start, cut_distance, cosp, sinp, df)
...
function get_threebody_fourier_ef(x, field, neighbors, order, power, cut_start, cut_distance, &
    ...
    do j = 2, neighbors
        do k = j+1, neighbors
            ...
            ksi3 = calc_ksi3_ef(X(:,:), field, j, k, neighbors, power, cut_start, cut_distance)
            theta = calc_angle(x(3:5, j), x(3:5, 1), x(3:5, k))
...
function calc_ksi3_ef(X, field, j, k, neighbors, power, cut_start, cut_distance) result(ksi3)
